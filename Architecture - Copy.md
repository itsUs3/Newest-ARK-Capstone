...
The architecture is modular at the service class level, but runtime orchestration is centralized in a single API gateway module (`backend/main.py`), which acts as composition root and routing layer.
...
flowchart LR
  ...
  FE -->|Axios /api| BE[FastAPI Backend main.py]
...
- Add online evaluation dashboards for ranking quality, fraud precision/recall, and RAG retrieval precision.
